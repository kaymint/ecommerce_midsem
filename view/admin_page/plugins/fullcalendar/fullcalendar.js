...
// overcome sucky customer_view-option-hash and option-merging behavior messing with options it shouldn't
function isForcedAtomicOption(name) {
	// Any option that ends in "Time" or "Duration" is probably a Duration,
	// and these will commonly be specified as plain objects, which we don't want to mess up.
	return /(Time|Duration)$/.test(name);
}
// FIX: find a different solution for customer_view-option-hashes and have a whitelist
...
var Popover = Class.extend({
	...
	position: function() {
		...
		// constrain to the customer_view port. if constrained by two edges, give precedence to top/left
...
var RowRenderer = Class.extend({
	...
	isRTL: null, // shortcut to the customer_view's isRTL option
...
Grid.mixin({
	...
	segDragMousedown: function(seg, ev) {
		...
		var mouseFollower = new MouseFollower(seg.el, {
			...
			zIndex: 2 // one above the .fc-customer_view
		});

		// Tracks mouse movement over the *customer_view's* coordinate map. Allows dragging and dropping between subcomponents
		// of the customer_view.
...
var DayGrid = Grid.extend({
	...
	numbersVisible: false, // should render a row for day/week numbers? set by outside customer_view. TODO: make internal
	bottomCoordPadding: 0, // hack for extending the hit area for the last row of the coordinate grid
	breakOnWeeks: null, // should create a new row for each week? set by outside customer_view
	...
	// Relies on the customer_view's colCnt and rowCnt. In the future, this component should probably be self-sufficient.
	...
	renderMoreLink: function(cell, hiddenSegs) {
		...
		return $('<a class="fc-more"/>')
			...
			.on('click', function(ev) {
				...
				else if (typeof clickOption === 'string') { // a customer_view name
...
var TimeGrid = Grid.extend({
	...
	// Relies on the customer_view's colCnt. In the future, this component should probably be self-sufficient.
	render: function() {
	...
	slatRowHtml: function() {
		...
		var slotDate; // will be on the customer_view's first day, but we only care about its time
		var minutes;
		var axisHtml;
...
var View = fc.View = Class.extend({
	...
	type: null, // subclass' customer_view name (string)
	name: null, // deprecated. use `type` instead

	calendar: null, // owner Calendar object
	options: null, // customer_view-specific options
	coordMap: null, // a CoordMap object for converting pixel regions to dates
	el: null, // the customer_view's containing element. set by Calendar

	// range the customer_view is actually displaying (moments)
	start: null,
	end: null, // exclusive

	// range the customer_view is formally responsible for (moments)
	// may be different from start/end. for example, a month customer_view might have 1st-31st, excluding padded dates
	intervalStart: null,
	...
	opt: function(name) {
		...
		val = this.options[name]; // look at customer_view-specific options first
		if (val !== undefined) {
			return val;
		}

		val = this.calendar.options[name];
		if ($.isPlainObject(val) && !isForcedAtomicOption(name)) { // customer_view-option-hashes are deprecated
	...
	// Triggers handlers that are customer_view-related. Modifies args before passing to calendar.
	trigger: function(name, thisObj) { // arguments beyond thisObj are passed along
		...
		return calendar.trigger.apply(
			...
			[name, thisObj || this].concat(
				...
				[ this ] // always make the last argument a reference to the customer_view. TODO: deprecate
			)
		);
	},
	...
	// Computes what the title at the top of the calendar should be for this customer_view
	...
	// Renders the customer_view inside an already-defined `this.el`
	render: function() {
		// subclasses should implement
	},
	...
	// Clears the customer_view's rendering
	destroy: function() {
	...
	// if `isAuto` is set to true, height becomes merely a suggestion and the customer_view should use its "natural" height.
	...
	// Given the total height of the customer_view, return the number of pixels that should be used for the scroller.
	// By default, uses this.scrollerEl, but can pass this in as well.
	// Utility for subclasses.
	computeScrollerHeight: function(totalHeight, scrollerEl) {
		var both;
		var otherHeight; // cumulative height of everything that is not the scrollerEl in the customer_view (header+borders)
	...
	// Should be called after we know the customer_view's dimensions have been restored following some type of destructive
	...
	// Renders the events onto the customer_view.
	...
	// Removes event elements from the customer_view.
	destroyEvents: function() {
		// subclasses should implement
	},
	...
	// The `this` value of the callback function will be the customer_view.
	...
	// Retrieves all the rendered segment objects for the customer_view
	getEventSegs: function() {
		// subclasses must implement
		return [];
	},
	...
	// Must be called when an event in the customer_view is dropped onto new location.
	...
	// Must be called when an event in the customer_view has been resized to a new length
	...
	// Selects a date range on the customer_view. `start` and `end` are both Moments.
	...
	function initialRender() {
		...
		content = $("<div class='fc-customer_view-container'/>").prependTo(element);
	...
	// Renders a customer_view because of a date change, customer_view-type change, or for the first time
	function renderView(delta, viewType) {
		ignoreWindowResize++;

		// if viewType is changing, destroy the old customer_view
		if (currentView && viewType && currentView.type !== viewType) {
			header.deactivateButton(currentView.type);
			freezeContentHeight(); // prevent a scroll jump when customer_view element is removed
		...
		// if viewType changed, or the customer_view was never created, create a fresh customer_view
		if (!currentView && viewType) {
			currentView = instantiateView(viewType);
			currentView.el =  $("<div class='fc-customer_view fc-" + viewType + "-customer_view' />").appendTo(content);
			header.activateButton(viewType);
		}

		if (currentView) {

			// let the customer_view determine what the delta means
			if (delta < 0) {
			...
			// render or rerender the customer_view
			if (
	...
	// Given a customer_view name for a custom customer_view or a standard customer_view, creates a ready-to-go View object
	...
	// Gets information about how to create a customer_view
	function getViewSpec(requestedViewType) {
		...
		// iterate up a customer_view's spec ancestor chain util we find a class to instantiate
		while (viewType && !viewClass) {
			viewOptions = {}; // only for this specific customer_view in the ancestry
		...
		if (viewClass) {
			...
			// options that are specified per the customer_view's duration, like "week" or "day"
			if (unitIsSingle && hash[unit]) {
				viewOptions = $.extend({}, hash[unit], viewOptions); // lowest priority
			}

			// compute the final text for the button representing this customer_view
			buttonText =
	...
	// Returns a boolean about whether the customer_view is okay to instantiate at some point
	function isValidViewType(viewType) {
		return Boolean(getViewSpec(viewType));
	}


	// Gets the text that should be displayed on a customer_view's button in the header
	...
	function windowResize(ev) {
		if (
			...
			currentView.start // customer_view has already been rendered
		) {
			if (updateSize(true)) {
	...
	-----------------------------------------------------------------------------*/
	// TODO: going forward, most of this stuff should be directly handled by the customer_view
	...
	// Forces navigation to a customer_view for the given date.
	// `viewType` can be a specific customer_view name or a generic one like "week" or "day".
	function zoomTo(newDate, viewType) {
		var viewStr;
		var match;

		if (!viewType || !isValidViewType(viewType)) { // a general customer_view name, or "auto"
			viewType = viewType || 'day';
			viewStr = header.getViewsWithButtons().join(' '); // space-separated string of all the views in the header

			// try to match a general customer_view name, like "week", against a specific one, like "agendaWeek"
			match = viewStr.match(new RegExp('\\w+' + capitaliseFirstLetter(viewType)));

			// fall back to the day customer_view being used in the header
			if (!match) {
			...
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						else if (calendar.isValidViewType(buttonName)) { // a customer_view type
							buttonClick = function() {
						...
						if (buttonClick) {
							...
							// smartProperty allows different text per customer_view button (ex: "Agenda Week" vs "Basic Week")
...
function EventManager(options) { // assumed to be a calendar
	...
	// Returns an array of events as to when the business hours occur in the given customer_view.
...
function backupEventDates(event) {
    ...
    /* An abstract class for the "basic" views, as well as month customer_view. Renders one or more rows of day cells.
----------------------------------------------------------------------------------------------------------------------*/
...
var BasicView = fcViews.basic = View.extend({
	...
	initialize: function() {
		...
		this.coordMap = this.dayGrid.coordMap; // the customer_view's date-to-cell mapping is identical to the subcomponent's
	},
	...
	// Renders the customer_view into `this.el`, which should already be assigned
	render: function() {
		...
		this.el.addClass('fc-basic-customer_view').html(this.renderHtml());
	...
	// Builds the HTML skeleton for the customer_view.
	...
	// Refreshes the horizontal dimensions of the customer_view
	updateWidth: function() {
		if (this.weekNumbersVisible) {
	...
	// Adjusts the vertical dimensions of the customer_view to the specified values
	...
	// Sets the height of just the DayGrid component in this customer_view
	...
	// Renders the given events onto the customer_view and populates the segments array
	renderEvents: function(events) {
	...
	// Retrieves all segment objects that are rendered in the customer_view
	getEventSegs: function() {
    ...
    /* A month customer_view with day cells running in rows (one-per-week) and columns
----------------------------------------------------------------------------------------------------------------------*/
...
MonthView.duration = { months: 1 };
    ...
    /* A week customer_view with simple day cells running horizontally
----------------------------------------------------------------------------------------------------------------------*/
...
fcViews.basicWeek = {
    ...
    /* A customer_view with a single simple day cell
----------------------------------------------------------------------------------------------------------------------*/
...
fcViews.agenda = View.extend({ // AgendaView
	...
	timeGrid: null, // the main time-grid subcomponent of this customer_view
	...
	initialize: function() {
		...
		if (this.opt('allDaySlot')) { // should we display the "all-day" area?
			this.dayGrid = new DayGrid(this); // the all-day subcomponent of this customer_view
	...
	// Renders the customer_view into `this.el`, which has already been assigned
	render: function() {

		this.el.addClass('fc-agenda-customer_view').html(this.renderHtml());
	...
	// Renders events onto the customer_view and populates the View's segment array
	renderEvents: function(events) {
		var dayEvents = [];
		var timedEvents = [];
		var daySegs = [];
    ...
    /* A week customer_view with an all-day cell area at the top, and a time grid below
----------------------------------------------------------------------------------------------------------------------*/
...
fcViews.agendaWeek = {
    ...
    /* A day customer_view with an all-day cell area at the top, and a time grid below
----------------------------------------------------------------------------------------------------------------------*/
